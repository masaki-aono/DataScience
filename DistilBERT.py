...
train_inputs, validation_inputs, train_labels, validation_labels, train_masks, validation_masks = t
...
for epoch_i in trange(epochs, desc="Epoch"):
  ...
  for step, batch in enumerate(train_dataloader):
    ...
    loss = loss_func(logits.view(-1,num_categories),b_labels.type_as(logits).view(-1,num_categories))
...
###############################################################################
  ...
  for i, batch in enumerate(validation_dataloader):
    ...
    with torch.no_grad():
      ...
      loss_func = BCEWithLogitsLoss() 
      loss = loss_func(b_logit_pred.view(-1,num_categories),b_labels.type_as(b_logit_pred).view(-1,num_categories))
      #convert labels to float for calculation
  ...
  torch.save(model.state_dict(), f'distilbert_model_reuters_epoch_{epoch_i}')


# In[18]:


torch.save(model.state_dict(), 'distilbert_model_reuters')
...
with open("distilbert_predicted.json", "w") as f:
